...
struct function<ReturnType(Args...)> {
  ...
  // explicit
  function(FunctionT f) : mInvoker(new callable<FunctionT>(f)) {}
  ...
  ReturnType operator()(Args... args) { return mInvoker->invoke(args...); }
  ...
  template <typename FunctionT> struct callable : public callable_base {
    callable(FunctionT func) : callable_base(), mFunction(func) {}

    virtual ReturnType invoke(Args... args) { return mFunction(args...); }

    virtual invoker_t clone() { return invoker_t(new callable(mFunction)); }
